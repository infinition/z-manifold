...
subspace, the attack is blocked (8 percent success versus 100 for LoRA) exactly
when its target behavior is unlike anything in the pool, and only partially
blocked when the target coincides with a common pool behavior, so the boundary
is real but pool-relative and predictable a priori. The mechanism trades peak
...
predicted by the same principle as the rest of the paper: the backdoor is
blocked exactly when its target behavior lies outside the pool's common
directions. On amazon the target ``output negative regardless of sentiment''
contradicts every sentiment adapter, so no $z$ expresses it; on social\_iqa the
task is itself a yes/no validity judgment, ``always No'' is a frequent pool
direction, and the trigger latches onto it. The security boundary is therefore
real but pool-relative, and its strength is predictable a priori from whether
the feared behavior is representable in the pool. This turns a binary claim
(``it blocks backdoors'') into a characterized one (``it blocks backdoors whose
...
independently trained adapters. Section~\ref{sec:controls} passes three controls
(dataset-family holdout, random subspace, and strong-regularization LoRA). A
DoRA variant and EigenLoRAx as a direct comparison remain to be run.
